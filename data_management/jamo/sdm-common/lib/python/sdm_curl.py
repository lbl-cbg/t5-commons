"""
    ...
    curl.py is a script containing Curl class that makes curl calls easy
...
class CurlHttpException(Exception):
    ...
    def __init__(self, httpError):
        self.response = [line.decode('utf-8') for line in httpError.readlines()]
...
class Curl(object):
    ...
    def __call(self, url, method, data=None, output='json', contenttype=None, verify=None, return_http_status_code=False):
        ...
        if output == 'json':
            try:
                ...
                # SMRTLink Curl is returning some xml here
                d = response
            return d
    ...
    # def toStruct(self, data: Any) -> 'SimpleNamespace':
...
class MultiPartForm(object):
    ...
    def __init__(self):
        self.form_fields = []
        self.files = []
        self.boundary = ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(40))
        return
