...
class QueueManager(object):
    ...
    def add_to_resource_list(self, queue):
        for key, a_resources in list(self.k_r_map.items()):
    ...
    def __create_queue_list(self, resources):
        ...
        for _id, queue in list(self.queues.items()):
    ...
    def status(self):
        ...
        for queue_id, queue in list(self.queues.items()):
            ret.append({'queue': queue.name, 'queued': queue.get_queued(), 'failed': queue.get_failed(),
                        'in_progress': queue.get_working(), 'status': queue.status})
        return ret
